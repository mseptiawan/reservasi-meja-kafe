<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Silahkan pilih nomor meja favoritmu yang tersedia di Senja Space</p>
        </div>
    </x-slot>

    <!-- Grid Layout Menampilkan Semua Card Meja -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @forelse($tables as $table)
            <div class="bg-white rounded-lg border border-slate-200/60 overflow-hidden flex flex-col justify-between shadow-sm">
                ...
                <div>
                    ...
                    <div class="relative h-44 bg-slate-50 overflow-hidden border-b border-slate-100">
                        ...
                        <!-- Badge Lokasi Area (Indoor/Outdoor/VIP) -->
                        <span class="absolute top-3 left-3 px-2 py-0.5 text-[10px] font-bold text-white bg-slate-900/70 backdrop-blur-sm rounded-md uppercase tracking-wider">
                            {{ $table->area }}
                        </span>
                    </div>
                    ...
                    <div class="p-4">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="flex items-center gap-1 text-[11px] font-bold text-slate-600">
                ...
                <!-- Tombol Aksi di Bawah Card Berdasarkan Status Meja -->
                <div class="px-4 py-3 border-t border-slate-100 bg-slate-50/50">
                    ...
                    @elseif($table->status === 'reserved' || $table->status === 'occupied')
                        <!-- Meja Penuh / Sedang Dipakai: Tombol Mati -->
                        <button disabled class="w-full py-2 bg-amber-100 text-amber-600 font-bold text-xs rounded-lg cursor-not-allowed border border-amber-200/50 flex items-center justify-center gap-1.5">
                            <i class="fa-solid fa-lock text-[10px]"></i> Sudah Dipesan
                        </button>
                    @else
                        <!-- Meja Rusak / Maintenance -->
                        <button disabled class="w-full py-2 bg-rose-50 text-rose-500 font-bold text-xs rounded-lg cursor-not-allowed border border-rose-100 flex items-center justify-center gap-1.5">
        ...
        @empty
            <div class="col-span-full bg-white rounded-lg border border-slate-200/60 p-10 text-center">
                <p class="text-xs text-slate-400">Maaf, saat ini belum ada meja kafe yang dikonfigurasi aktif oleh admin.</p>
            </div>
        @endforelse
    </div>
</x-app-layout>