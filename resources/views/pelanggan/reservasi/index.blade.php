            ...
            <div class="bg-white rounded-lg border border-slate-200/60 overflow-hidden flex flex-col justify-between shadow-sm">
                ...
                <div>
                    <div class="relative h-44 bg-slate-50 overflow-hidden border-b border-slate-100">
                        ...
                        <span class="absolute top-3 left-3 px-2 py-0.5 text-[10px] font-medium text-white bg-slate-900/70 backdrop-blur-sm rounded-md uppercase tracking-wider">
                            {{ $table->area }}
                        </span>
                    </div>
                ...
                <div class="px-4 py-3 border-t border-slate-100 bg-slate-50/50">
                    @if($table->status === 'available')
                        <a href="{{ route('reservasi.create', $table->id) }}" class="block w-full text-center py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs rounded-lg transition-colors">
                            Booking Meja Ini
                        </a>
                    @elseif($table->status === 'reserved' || $table->status === 'occupied')
                        <button disabled class="w-full py-2 bg-amber-100 text-amber-600 font-medium text-xs rounded-lg cursor-not-allowed border border-amber-200/50 flex items-center justify-center gap-1.5">
                            <i class="fa-solid fa-lock text-[10px]"></i> Sudah Dipesan
                        </button>
                    @else
                        <button disabled class="w-full py-2 bg-rose-50 text-rose-500 font-medium text-xs rounded-lg cursor-not-allowed border border-rose-100 flex items-center justify-center gap-1.5">
