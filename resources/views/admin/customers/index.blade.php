        ...
        <x-slot name="header">
            <div
                ...
                <x-page-header title="Daftar Pelanggan Aktif"
                    subtitle="Daftar seluruh pelanggan PesanMeja yang berstatus aktif dan terverifikasi">
                ...
                <form action="{{ route('admin.customers.index') }}" method="GET"
                    class="w-full md:w-72 m-0 flex gap-2 shrink-0">
                    ...
                    @if ($search)
                        <a href="{{ route('admin.customers.index') }}"
                            class="h-9 px-2.5 bg-slate-100 text-slate-500 hover:text-slate-700 rounded-lg flex items-center justify-center text-xs border border-slate-200 transition-colors shrink-0">
                            Reset
                        </a>
                    @endif
                </form>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white font-medium text-slate-600">
                        @forelse($customers as $customer)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-4 px-6">
                                    <span
                                        class="font-semibold text-slate-800 block text-[13px]">{{ $customer->name }}</span>
                                    <span
                                        class="text-[10px] font-medium text-slate-400 inline-block mt-0.5">{{ $customer->email }}</span>
                                </td>
                                ...
                                <td class="py-4 px-6">
                                    <span
                                        class="text-[10px] font-mono font-semibold bg-indigo-50 border border-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-md inline-block">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 text-center text-slate-400 font-medium">Tidak ada data
                                    pelanggan aktif ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                @forelse($customers as $customer)
                    <div class="p-4 space-y-3 bg-white">
                        ...
                        <div class="text-[11px] font-medium text-slate-400 pt-0.5">
                            <i class="fa-solid fa-calendar mr-1"></i>Bergabung:
