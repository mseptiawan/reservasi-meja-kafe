<x-app-layout>
    ...
    <div class="flex items-center justify-between mb-5">
        <div class="text-xs text-slate-500 font-medium">
            Total Pengumuman: <span class="text-slate-800 font-bold">{{ $announcements->count() }} Konten</span>
        </div>
     ...
     class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50">
                                ...
                                <span class="text-[9px] uppercase font-bold mt-1.5 tracking-wider">No Banner</span>
                            </div>
                        @endif
                        ...
                        <div class="absolute top-3 left-3">
                            ...
                            @endphp
                            <span class="px-2 py-0.5 text-[9px] font-bold uppercase rounded border {{ $typeClasses[$announcement->type] ?? $typeClasses['announcement'] }}">
                        ...
                        <div class="absolute top-3 right-3">
                            ...
                            @endphp
                            <span class="px-2 py-0.5 text-[9px] font-bold uppercase rounded {{ $statusClasses[$announcement->status] }}">
                                {{ $announcement->status }}
                            </span>
                        </div>
                    </div>
                    ...
                    <!-- Informasi Konten -->
                    <div class="p-4">
                        <h3 class="font-bold text-sm text-slate-800 line-clamp-1 mb-1" title="{{ $announcement->title }}">
                            {{ $announcement->title }}
                        </h3>
    ...
    <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-100 text-slate-400 hover:text-rose-600 transition-colors" title="Hapus">
        ...
        @empty
            <div class="col-span-full bg-white rounded-lg border border-slate-200/60 p-10 text-center flex flex-col items-center justify-center">
                ...
                <h3 class="font-bold text-slate-700 text-sm">Belum ada pengumuman</h3>
