<x-app-layout>
    ...
    <x-slot name="header">
        <div class="border-b border-slate-100 pb-5 md:border-none md:pb-0">
            ...
            <x-page-header title="Data Pengumuman"
                subtitle="Kelola info internal, promo, event, dan info maintenance PesanMeja">
    ...
    <div class="flex items-center justify-between mb-5">
        ...
        @if (Auth::user()->role === 'admin')
            <a href="{{ route('admin.announcements.create') }}"
                class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-lg transition-all duration-150">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @forelse($announcements as $announcement)
            <div class="bg-white rounded-lg border border-slate-200/60 overflow-hidden flex flex-col justify-between">
                ...
                <div>
                    ...
                    <div class="relative h-40 bg-slate-50 overflow-hidden border-b border-slate-100">
                        @if ($announcement->image)
                            <img src="{{ \Illuminate\Support\Str::contains($announcement->image, ['http://', 'https://']) ? $announcement->image : asset('storage/' . $announcement->image) }}"
                                alt="{{ $announcement->title }}" class="w-full h-full object-cover">
                        @else
                            <div
                        ...
                        <div class="absolute top-3 left-3">
                            ...
                            @endphp
                            <span
                                class="px-2 py-0.5 text-[9px] font-medium uppercase rounded border {{ $typeClasses[$announcement->type] ?? $typeClasses['announcement'] }}">
                                {{ $typeLabels[$announcement->type] ?? $typeLabels['announcement'] }}
                            </span>
                        </div>

                        <!-- Badge Status (Hanya Muncul jika User adalah Admin) -->
                        @if (Auth::user()->role === 'admin')
                            <div class="absolute top-3 right-3">
                                ...
                                @endphp
                                <span
                                    class="px-2 py-0.5 text-[9px] font-medium uppercase rounded {{ $statusClasses[$announcement->status] }}">
                                    {{ $announcement->status }}
                                </span>
                            </div>
                        @endif
                    ...
                    <!-- Informasi Konten -->
                    <div class="p-4">
                        <h3 class="font-medium text-sm text-slate-800 line-clamp-1 mb-1"
                ...
                <!-- FOOTER CARD EDIT & HAPUS HANYA UNTUK ADMIN -->
                @if (Auth::user()->role === 'admin')
                    <div
                        class="px-4 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-1.5">
                        <!-- Tombol Edit -->
        ...
        @empty
            <div
                class="col-span-full bg-white rounded-lg border border-slate-200/60 p-10 text-center flex flex-col items-center justify-center">
                <div
                ...
                <p class="text-[11px] text-slate-400 mt-0.5 max-w-xs leading-relaxed">
                    {{ Auth::user()->role === 'admin' ? 'Mulai buat info promo, event, atau berita internal pertamamu sekarang.' : 'Nantikan kabar menarik, promo, dan info terbaru dari kami.' }}
                </p>
            </div>
        @endforelse
    </div>
