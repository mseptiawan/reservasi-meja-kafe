        ...
        <form method="POST" action="{{ route('admin.announcements.update', $announcement->id) }}"
            ...
            <div class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="md:col-span-2 space-y-1.5">
                        <label for="title" class="text-xs font-semibold text-slate-700 tracking-wide uppercase">Judul
                            Pengumuman <span class="text-rose-500">*</span></label>
                    ...
                    <div class="space-y-1.5">
                        <label for="type" class="text-xs font-semibold text-slate-700 tracking-wide uppercase">Tipe
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="announcement"
                                {{ old('type', $announcement->type) == 'announcement' ? 'selected' : '' }}>Umum</option>
                            <option value="promo" {{ old('type', $announcement->type) == 'promo' ? 'selected' : '' }}>
                                Promo</option>
                            <option value="event" {{ old('type', $announcement->type) == 'event' ? 'selected' : '' }}>
                                Event</option>
                            <option value="info_internal"
                                {{ old('type', $announcement->type) == 'info_internal' ? 'selected' : '' }}>Info
                                Internal</option>
                            <option value="maintenance"
                                {{ old('type', $announcement->type) == 'maintenance' ? 'selected' : '' }}>Maintenance
                            </option>
                        </select>
                    ...
                    <div class="space-y-1.5">
                        <label for="status"
                            class="text-xs font-semibold text-slate-700 tracking-wide uppercase">Status <span
                                class="text-rose-500">*</span></label>
                        <select id="status" name="status" required
                            class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-xs transition-all">
                            <option value="draft"
                                {{ old('status', $announcement->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                            <option value="published"
                                {{ old('status', $announcement->status) == 'published' ? 'selected' : '' }}>Terbitkan
                            </option>
                            <option value="archived"
                                {{ old('status', $announcement->status) == 'archived' ? 'selected' : '' }}>Arsip
                            </option>
                        </select>
                ...
                <div class="space-y-1.5">
                    <label for="content" class="text-xs font-semibold text-slate-700 tracking-wide uppercase">Isi
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-slate-700 tracking-wide uppercase">Banner Gambar
                        (Opsional)</label>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                        ...
                        <div id="preview-container"
                            ...
                            @if ($announcement->image)
                                <img id="image-preview" src="{{ asset('storage/' . $announcement->image) }}"
                        ...
                        <div class="md:col-span-3 space-y-1.5">
                            ...
                            <p class="text-[10px] text-slate-400">Format: JPG, JPEG, PNG, WEBP (Maksimal 2MB). Biarkan
                                kosong jika tidak ingin mengubah banner.</p>
            ...
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2">
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid fa-floppy-disk mr-1.5 text-[10px]"></i> Perbarui
                </button>
            </div>
        </form>
    </div>
