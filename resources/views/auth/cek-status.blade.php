...
<head>
    ...
    <title>Cek Status Akun | PesanMeja</title>
    ...
    <div class="w-full max-w-md bg-white border border-slate-100 rounded-xl shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <form method="POST" action="{{ route('account.status.check') }}" class="space-y-3.5">
            ...
            <div class="space-y-1.5">
                <label for="search_key"
                    class="text-[11px] font-semibold text-slate-600 tracking-wide uppercase">Email</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 text-xs">
                        <i class="fa-solid fa-magnifying-glass"></i>
                    </span>
                    <input id="search_key" type="text" name="search_key" value="{{ old('search_key') }}" required
        ...
        @if (session('account_user'))
            ...
            <div class="p-4 bg-slate-50 border border-slate-100 rounded-lg space-y-3">
                <div
                    class="text-[11px] font-medium text-slate-400 uppercase tracking-wider border-b border-slate-200/60 pb-1.5">
                    Hasil Pencarian</div>
                ...
                <div class="grid grid-cols-3 gap-y-2 text-xs">
                    ...
                    <span class="col-span-2 flex items-center gap-1">
                        ...
                        @if ($user['status_verifikasi'] === 'pending')
                            <span
                                class="px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 font-medium text-[10px] uppercase">
                                <i class="fa-solid fa-hourglass-half text-[9px] mr-0.5 animate-pulse"></i> Pending /
                                Menunggu Admin
                            </span>
                        @elseif ($user['status_verifikasi'] === 'active')
                            <span
                                class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium text-[10px] uppercase">
                                <i class="fa-solid fa-circle-check text-[9px] mr-0.5"></i> Aktif (Silakan Login)
                            </span>
                        @else
                            <span
                                class="px-2 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-200 font-medium text-[10px] uppercase">
                ...
                @if ($user['status_verifikasi'] === 'approved')
                    <div class="pt-2">
                        <a href="{{ route('login') }}"
                            class="w-full block text-center py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-md shadow-sm transition-all duration-150">
