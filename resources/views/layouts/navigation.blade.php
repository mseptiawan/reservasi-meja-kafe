            ...
            [
                ...
                'submenu' => [
                    [
                        ...
                        'url' => route('reservasi.history'),
                        'route_name' => 'reservasi.history',
    ...
    <nav id="sidebarNav" class="flex-1 px-4 md:px-2 xl:px-4 py-6 space-y-7 pb-10 custom-scrollbar">
        @foreach ($menus as $sectionLabel => $items)
            <section class="space-y-1.5">
                <p
                    class="text-[8px] text-slate-400 font-medium px-4 mb-2 uppercase tracking-[0.2em] md:hidden xl:block">
                    {{ $sectionLabel }}
                </p>
                        ...
                        <div x-data="{ open: {{ $isDropdownActive ? 'true' : 'false' }} }" class="space-y-1">
                            <button @click="open = !open"
                                ...
                                <i :class="open ? 'rotate-180' : ''"
                                    class="fa-solid fa-chevron-down text-[10px] transition-transform duration-200 md:hidden xl:block"></i>
                            </button>
                                ...
                                @foreach ($menu['submenu'] as $sub)
                                    @php
                                        ...
                                        if (isset($sub['route_name'])) {
                                            $isSubActive = request()->routeIs($sub['route_name']);
                                        } else {
                                            $isSubActive =
                                                request()->url() == $sub['url'] ||
                                                str_contains(request()->url(), strtok($sub['url'], '#'));
                                        }
                                    @endphp
                                    <a href="{{ $sub['url'] }}"
    ...
    <div class="p-4 md:p-2 xl:p-4 border-t border-slate-100 bg-white">
        <div
            ...
            <form method="POST" action="{{ route('logout') }}" class="ml-0 xl:ml-1.5 shrink-0 md:mt-2 xl:mt-0">
                @csrf
                <button type="submit" title="Keluar Aplikasi"
                    class="p-1.5 text-slate-400 hover:text-rose-600 rounded-xl hover:bg-rose-50 transition-all duration-150 cursor-pointer">
